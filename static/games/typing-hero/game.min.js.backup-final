! function(n, t) {
    ...
    return function(n) {
        ...
        function updateDifficulty() {
            ...
            if (newEnemySpeed !== M) {
                M = newEnemySpeed;
                console.log('敵機速度提升至:', M.toFixed(2), '難度等級:', difficultyLevel, '最後20秒:', isLast20Seconds);
            }
            
            // 敵機生成間隔遞減 (基礎間隔 4000ms，每級 -500ms，最後20秒大幅縮短)
            var newAddEnemyTime = Math.max(1000, 4000 - (difficultyLevel * 500));
            if (isLast20Seconds) {
                // 最後20秒：額外縮短到最低500ms
                newAddEnemyTime = Math.max(500, newAddEnemyTime - 500);
            }
            if (newAddEnemyTime !== c.addEnemyTime) {
                c.addEnemyTime = newAddEnemyTime;
                console.log('敵機生成間隔調整至:', c.addEnemyTime, 'ms，難度等級:', difficultyLevel, '最後20秒:', isLast20Seconds);
