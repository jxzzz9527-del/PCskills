! function(n, t) {
    ...
    return function(n) {
        ...
        n.exports = function(n) {
            ...
            function w(n, t, e, i) {
                1 == arguments.length ? (t = n.def, e = n.valid, i = n.placeholder, n = on.checkArg(n.text, "CHINESE" == on.language ? "請輸入：" : "Please input:")) : n = on.checkArg(n, "CHINESE" == on.language ? "請輸入：" : "Please input:"), on.id("jetInputContent").append(on.ct("div").addClass("jet-input-text").txt(n));
            ...
            function L(n, t, e) {
                ...
                if (n.select("[jet-valid]").each((function(n) {
                    ...
                    r && o(e)(i, n);
                    var s = "CHINESE" == on.language ? "輸入有誤，請按提示改正。" : "Values is not expected";
            ...
            var en = {
                ...
                input: function(n, t, e) {
                    ...
                    if (i.exist() || (function() {
                            ...
                            on.body().append(h)
                        }(), i = on.id("jetInputContent")), i.empty(), "string" == on.type(n)) i.append(on.ct("div").txt(c(n, "CHINESE" == this.language ? "資訊輸入" : "Input Information"))), w("CHINESE" == this.language ? "請輸入：" : "Please input:", null, null, null);
                    else if ("array" == on.type(n)) i.append(on.ct("div").txt(c(n[0], "CHINESE" == this.language ? "資訊輸入" : "Input Information"))), w(n[1], n[2], n[3], n[4]), null != n[3] && this.initValid(i);
                    else {
                        i.append(on.ct("div").txt(c(n.title, "CHINESE" == this.language ? "資訊輸入" : "Input Information")));
                        var r = c(n.def, []),
                            h = c(n.valid, []),
                            s = c(n.placeholder, []);
                        if (null == n.text || "string" == on.type(n.text)) w(n.text, r, h, s);
                        else
                            for (var u = 0; u < n.text.length; u++) w(c(n.text[u], "CHINESE" == this.language ? "請輸入：" : "Please input:"), r[u], h[u], s[u]);
        ...
        function initScreens() {
            ...
            console.log('startScreen display:', computedStyle.display);
            console.log('startScreen position:', computedStyle.position);
            ...
            if (startScreen && gameWrapper) {
                ...
                // 更新開始畫面的最高分
                if (startBestScore && bestScore) {
                    startBestScore.textContent = bestScore.textContent;
                }
            } else {
        ...
        function startGame() {
            ...
            if (startScreen && gameWrapper) {
                ...
                const checkImagesLoaded = () => {
                    if (i.complete && r[0] && r[0].complete) {
                        
                        // 確保遊戲尺寸正確設定
                        if (g.gameWidth === 0 || g.gameHeight === 0) {
                            const gameWrapperWidth = window.innerWidth > 1200 ? 1000 : window.innerWidth - 20;
                            const gameWrapperHeight = 911; // 固定高度
                            g.gameWidth = gameWrapperWidth;
                            g.gameHeight = gameWrapperHeight;
                            console.log('重新設定遊戲尺寸:', gameWrapperWidth, 'x', gameWrapperHeight);
                        }
                        
                        // 開始遊戲邏輯
                        if (typeof geneEnemy === 'function') {
                            geneEnemy();
                        }
                        // 啟動遊戲循環
                        D();
        ...
        function showEndScreen() {
            ...
            if (gameWrapper && endScreen) {
                ...
                if (finalBestScore && bestScore) {
                    finalBestScore.textContent = bestScore.textContent;
                }
                if (finalEnemies) {
                    // 計算擊毀敵機數（這裡需要根據實際遊戲邏輯調整）
                    finalEnemies.textContent = '0'; // 暫時設為0
                }
                
                // 獲取當前分數
                let playerScore = 0;
                if (score) {
                    playerScore = parseInt(score.textContent) || 0;
                }
                
                console.log('第一個showEndScreen函數 - 分數:', playerScore);
                
                // 提交分數到排行榜
                if (playerScore > 0) {
                    console.log('準備提交分數到排行榜:', { playerScore, playTime: 60 });
                    ...
                    if (window.parent && window.parent !== window) {
                        ...
                        console.log('不在iframe中，直接調用分數提交函數');
                        // 如果不在iframe中，直接調用分數提交函數
                        if (typeof submitScoreToLeaderboard === 'function') {
                            submitScoreToLeaderboard(playerScore, playTime);
                        } else {
                            console.log('submitScoreToLeaderboard函數不存在');
                        }
                    }
                } else {
                    console.log('分數為0或無效，跳過提交:', playerScore);
                }
                
                // 獲取並顯示個人最高分
                fetchAndDisplayPersonalBest(playerScore);
            }
        }
            ...
            return t = n, (e = [{
                ...
                value: function() {
                    if (c.ctx.save(), this.isDie) {
                        ...
                        c.ctx.translate(n, t), c.ctx.rotate(this.deg), c.ctx.translate(-n, -t), this.dieImg[this.dieIndex] && this.dieImg[this.dieIndex].complete && c.ctx.drawImage(this.dieImg[this.dieIndex], n - this.dieLen / 2, t - this.dieLen / 2, this.dieLen, this.dieLen)
                    } else c.ctx.translate(this.x, this.y), c.ctx.rotate(this.deg), c.ctx.translate(-this.x, -this.y), this.img && this.img.complete && c.ctx.drawImage(this.img, this.x - this.w / 2, this.y - this.h / 2, this.w, this.h);
                    c.ctx.restore()
                }
            }, {
                key: "findTarget",
                value: function() {
                    ...
                    return c.player.setErr(), m("沒有找到目標", !0, 1e3), null
                }
            }, {
                key: "touchTarget",
                value: function() {
        ...
        var q = function() {
            ...
            return t = n, (e = [{
                ...
                key: "draw",
                value: function() {
                    this.img && this.img[this.index] && this.img[this.index].complete && this.ctx.drawImage(this.img[this.index], this.x - this.w / 2, this.y - this.h / 2, this.w, this.h)
                }
            }, {
                key: "resetPosAndSize",
                value: function() {
        ...
        var L = function() {
            function n() {
                ...
                this.setBestScore(t ? parseInt(t) : 0), this.protect = 1, this.isClear = !1, this.clearPer = 15, this.clearIndex = 0, this.clearMax = 600, this.gameTime = 60, this.startTime = Date.now(), this.pauseStartTime = 0, this.totalPauseTime = 0, this.tailY = this.y + this.h / 2 - 6, this.fireAni = new q({
                    ctx: c.ctx,
                    host: this,
                    img: u,
                    pos: function(n) {
            ...
            return t = n, (e = [{
                ...
                value: function() {
                    // 護盾效果已移到遊戲循環中繪製
                    c.ctx.save(), c.ctx.translate(this.x, this.y), c.ctx.rotate(this.deg), c.ctx.translate(-this.x, -this.y), this.err_i > -1 ? (this.err_img[this.err_i] && this.err_img[this.err_i].complete && c.ctx.drawImage(this.err_img[this.err_i], this.x - this.w / 2, this.y - this.h / 2, this.w, this.h)) : (this.img && this.img.complete && c.ctx.drawImage(this.img, this.x - this.w / 2, this.y - this.h / 2, this.w, this.h)), this.fireAni.draw(), c.ctx.restore()
                }
            }, {
                key: "resetPos",
                value: function() {
                ...
                value: function(n) {
                    if (null == n) {
                        // 新的分數計算方式
                        this.score++;
                    } else if (n && n.bopomofo) {
                        // 根據注音量和距離計算分數
                        var bopomofoCount = n.bopomofo.length; // 注音數量
                        var distance = Math.sqrt(Math.pow(n.x - this.x, 2) + Math.pow(n.y - this.y, 2)); // 距離
                        
                        // 分數計算公式：基礎分數 + 注音量獎勵 + 距離獎勵
                        var baseScore = 10; // 基礎分數
                        var bopomofoBonus = bopomofoCount * 5; // 每個注音 +5 分
                        var distanceBonus = Math.floor(distance / 10); // 每10像素距離 +1 分
                        
                        var totalScore = baseScore + bopomofoBonus + distanceBonus;
                        this.score += totalScore;
                        
                        console.log('分數計算:', {
                            bopomofo: n.bopomofo,
                            bopomofoCount: bopomofoCount,
                            distance: Math.floor(distance),
                            baseScore: baseScore,
                            bopomofoBonus: bopomofoBonus,
                    ...
                    l.a.id("score").txt(this.score), this.score > this.bestScore && this.setBestScore(this.score)
                }
            }, {
                key: "setBestScore",
                value: function(n) {
                ...
                key: "restart",
                value: function() {
                    this.setLetterNum(0), this.setScore(0), this.setProtect(1), this.err_i = -1, this.deg = 0, this.target = null, this.isClear = !1, this.clearIndex = 0, this.clearPer = 15, this.clearMax = 600, this.gameTime = 60, this.startTime = Date.now(), this.pauseStartTime = 0, this.totalPauseTime = 0
                }
            }, {
                key: "getRemainingTime",
                value: function() {
                    var elapsed = Math.floor((Date.now() - this.startTime - this.totalPauseTime) / 1000);
        ...
        var questionBank = {
            level1: [
                { word: "佛", bopomofo: "ㄈㄛˊ" },
                { word: "泰", bopomofo: "ㄊㄞˋ" },
                { word: "廟", bopomofo: "ㄇㄧㄠˋ" },
                { word: "華", bopomofo: "ㄏㄨㄚˊ" },
                { word: "尚", bopomofo: "ㄕㄤˋ" },
                { word: "逛", bopomofo: "ㄍㄨㄤˋ" },
                { word: "埃", bopomofo: "ㄞ" },
                { word: "羅", bopomofo: "ㄌㄨㄛˊ" },
                { word: "幻", bopomofo: "ㄏㄨㄢˋ" },
                { word: "旋", bopomofo: "ㄒㄩㄢˊ" },
                { word: "扁", bopomofo: "ㄅㄧㄢˇ" },
                { word: "瞬", bopomofo: "ㄕㄨㄣˋ" },
                { word: "驗", bopomofo: "ㄧㄢˋ" },
                { word: "牙", bopomofo: "ㄧㄚˊ" },
                { word: "芝", bopomofo: "ㄓ" },
                { word: "盜", bopomofo: "ㄉㄠˋ" },
                ...
                { word: "潮", bopomofo: "ㄔㄠˊ" },
                { word: "鬱", bopomofo: "ㄩˋ" },
                { word: "划", bopomofo: "ㄏㄨㄚˊ" },
                { word: "邁", bopomofo: "ㄇㄞˋ" },
                { word: "困", bopomofo: "ㄎㄨㄣˋ" },
                { word: "龐", bopomofo: "ㄆㄤˊ" },
                { word: "搓", bopomofo: "ㄘㄨㄛ" },
                { word: "榮", bopomofo: "ㄖㄨㄥˊ" },
                { word: "富", bopomofo: "ㄈㄨˋ" },
                { word: "遍", bopomofo: "ㄅㄧㄢˋ" },
                { word: "背", bopomofo: "ㄅㄟˋ" },
                { word: "錶", bopomofo: "ㄅㄧㄠˇ" },
                { word: "掉", bopomofo: "ㄉㄧㄠˋ" },
                { word: "深", bopomofo: "ㄕㄣ" },
                { word: "鎖", bopomofo: "ㄙㄨㄛˇ" },
                { word: "鑰", bopomofo: "ㄧㄠˋ" },
                { word: "匙", bopomofo: "ㄔˊ" },
                { word: "汁", bopomofo: "ㄓ" },
                { word: "妻", bopomofo: "ㄑㄧ" },
                ...
                { word: "扇", bopomofo: "ㄕㄢˋ" },
                { word: "遲", bopomofo: "ㄔˊ" },
                { word: "靈", bopomofo: "ㄌㄧㄥˊ" },
                { word: "鎚", bopomofo: "ㄔㄨㄟˊ" },
                { word: "嘖", bopomofo: "ㄗㄜˊ" },
                { word: "足", bopomofo: "ㄗㄨˊ" },
                { word: "蹈", bopomofo: "ㄉㄠˇ" },
                { word: "蹦", bopomofo: "ㄅㄥˋ" },
                ...
                { word: "適", bopomofo: "ㄕˋ" },
                { word: "賓", bopomofo: "ㄅㄧㄣ" },
                { word: "盡", bopomofo: "ㄐㄧㄣˋ" },
                { word: "社", bopomofo: "ㄕㄜˋ" },
                { word: "與", bopomofo: "ㄩˇ" },
                { word: "遙", bopomofo: "ㄧㄠˊ" },
                { word: "投", bopomofo: "ㄊㄡˊ" },
                { word: "巧", bopomofo: "ㄑㄧㄠˇ" },
                { word: "彈", bopomofo: "ㄊㄢˊ" },
                { word: "遨", bopomofo: "ㄠˊ" },
                { word: "緩", bopomofo: "ㄏㄨㄢˇ" },
                { word: "陀", bopomofo: "ㄊㄨㄛˊ" },
                { word: "泳", bopomofo: "ㄩㄥˇ" },
                { word: "串", bopomofo: "ㄔㄨㄢˋ" },
                { word: "般", bopomofo: "ㄅㄢ" },
                { word: "姿", bopomofo: "ㄗ" },
                { word: "溜", bopomofo: "ㄌㄧㄡ" },
                { word: "耳", bopomofo: "ㄦˇ" },
                { word: "鷹", bopomofo: "ㄧㄥ" },
                { word: "滑", bopomofo: "ㄏㄨㄚˊ" },
                { word: "煩", bopomofo: "ㄈㄢˊ" },
                { word: "曾", bopomofo: "ㄘㄥˊ" },
                { word: "揮", bopomofo: "ㄏㄨㄟ" },
                { word: "彼", bopomofo: "ㄅㄧˇ" },
                { word: "秒", bopomofo: "ㄇㄧㄠˇ" },
                { word: "宣", bopomofo: "ㄒㄩㄢ" },
                { word: "優", bopomofo: "ㄧㄡ" },
                { word: "弧", bopomofo: "ㄏㄨˊ" },
                { word: "四", bopomofo: "ㄙˋ" },
                ...
                { word: "湯", bopomofo: "ㄊㄤ" },
                { word: "拋", bopomofo: "ㄆㄠ" },
                { word: "糧", bopomofo: "ㄌㄧㄤˊ" },
                { word: "姐", bopomofo: "ㄐㄧㄝˇ" },
                { word: "爭", bopomofo: "ㄓㄥ" },
                { word: "警", bopomofo: "ㄐㄧㄥˇ" },
                { word: "偷", bopomofo: "ㄊㄡ" },
                { word: "暫", bopomofo: "ㄓㄢˋ" },
                { word: "偕", bopomofo: "ㄐㄧㄝ" },
                { word: "醫", bopomofo: "ㄧ" },
                { word: "命", bopomofo: "ㄇㄧㄥˋ" },
                { word: "奉", bopomofo: "ㄈㄥˋ" },
                { word: "獻", bopomofo: "ㄒㄧㄢˋ" },
                { word: "療", bopomofo: "ㄌㄧㄠˊ" },
                { word: "衛", bopomofo: "ㄨㄟˋ" },
                { word: "藥", bopomofo: "ㄧㄠˋ" },
                { word: "治", bopomofo: "ㄓˋ" },
                { word: "翻", bopomofo: "ㄈㄢ" },
                { word: "嶺", bopomofo: "ㄌㄧㄥˇ" },
                { word: "捐", bopomofo: "ㄐㄩㄢ" },
                { word: "宿", bopomofo: "ㄙㄨˋ" },
                { word: "堂", bopomofo: "ㄊㄤˊ" },
                { word: "農", bopomofo: "ㄋㄨㄥˊ" },
                { word: "視", bopomofo: "ㄕˋ" },
                { word: "注", bopomofo: "ㄓㄨˋ" },
                { word: "憑", bopomofo: "ㄆㄧㄥˊ" },
                { word: "秀", bopomofo: "ㄒㄧㄡˋ" },
                { word: "稱", bopomofo: "ㄔㄥ" },
                { word: "朗", bopomofo: "ㄌㄤˇ" },
                { word: "登", bopomofo: "ㄉㄥ" },
                { word: "穆", bopomofo: "ㄇㄨˋ" },
                ...
                { word: "峰", bopomofo: "ㄈㄥ" },
                { word: "母", bopomofo: "ㄇㄨˇ" },
                { word: "六", bopomofo: "ㄌㄧㄡˋ" },
                { word: "酷", bopomofo: "ㄎㄨˋ" },
                { word: "磨", bopomofo: "ㄇㄛˊ" },
                { word: "攀", bopomofo: "ㄆㄢ" },
                { word: "氧", bopomofo: "ㄧㄤˇ" },
                { word: "訓", bopomofo: "ㄒㄩㄣˋ" },
                { word: "谷", bopomofo: "ㄍㄨˇ" },
                { word: "響", bopomofo: "ㄒㄧㄤˇ" },
                { word: "薄", bopomofo: "ㄅㄠˊ" },
                { word: "烙", bopomofo: "ㄌㄠˋ" },
                { word: "沾", bopomofo: "ㄓㄢ" },
                { word: "咖", bopomofo: "ㄎㄚ" },
                { word: "里", bopomofo: "ㄌㄧˇ" },
                { word: "印", bopomofo: "ㄧㄣˋ" },
                { word: "甩", bopomofo: "ㄕㄨㄞˇ" },
                { word: "材", bopomofo: "ㄘㄞˊ" },
                { word: "臨", bopomofo: "ㄌㄧㄣˊ" },
                { word: "樓", bopomofo: "ㄌㄡˊ" },
                { word: "頸", bopomofo: "ㄐㄧㄥˇ" },
                { word: "代", bopomofo: "ㄉㄞˋ" },
                { word: "濟", bopomofo: "ㄐㄧˋ" },
                { word: "阻", bopomofo: "ㄗㄨˇ" },
                { word: "尼", bopomofo: "ㄋㄧˊ" },
                { word: "器", bopomofo: "ㄑㄧˋ" },
                { word: "醬", bopomofo: "ㄐㄧㄤˋ" },
                { word: "義", bopomofo: "ㄧˋ" },
                { word: "麵", bopomofo: "ㄇㄧㄢˋ" },
                { word: "郁", bopomofo: "ㄩˋ" },
                { word: "壽", bopomofo: "ㄕㄡˋ" },
                { word: "醋", bopomofo: "ㄘㄨˋ" },
                { word: "未", bopomofo: "ㄨㄟˋ" },
                { word: "震", bopomofo: "ㄓㄣˋ" },
                { word: "京", bopomofo: "ㄐㄧㄥ" },
                { word: "晴", bopomofo: "ㄑㄧㄥˊ" },
                { word: "塔", bopomofo: "ㄊㄚˇ" },
                { word: "訊", bopomofo: "ㄒㄩㄣˋ" },
                { word: "吉", bopomofo: "ㄐㄧˊ" },
                { word: "橋", bopomofo: "ㄑㄧㄠˊ" },
                { word: "搬", bopomofo: "ㄅㄢ" },
                { word: "恨", bopomofo: "ㄏㄣˋ" },
                { word: "拒", bopomofo: "ㄐㄩˋ" },
                { word: "尷", bopomofo: "ㄍㄢ" },
                ...
                { word: "繆", bopomofo: "ㄇㄡˊ" },
                { word: "毋", bopomofo: "ㄨˊ" },
                { word: "掘", bopomofo: "ㄐㄩㄝˊ" },
                { word: "纏", bopomofo: "ㄔㄢˊ" },
                { word: "鼎", bopomofo: "ㄉㄧㄥˇ" },
                { word: "擠", bopomofo: "ㄐㄧˇ" },
                { word: "颳", bopomofo: "ㄍㄨㄚ" },
                { word: "廚", bopomofo: "ㄔㄨˊ" },
                { word: "淋", bopomofo: "ㄌㄧㄣˊ" },
                { word: "晒", bopomofo: "ㄕㄞˋ" },
                { word: "覆", bopomofo: "ㄈㄨˋ" },
                { word: "揭", bopomofo: "ㄐㄧㄝ" },
                { word: "培", bopomofo: "ㄆㄟˊ" },
                { word: "謁", bopomofo: "ㄧㄝˋ" },
                { word: "卓", bopomofo: "ㄓㄨㄛˊ" },
                { word: "耕", bopomofo: "ㄍㄥ" },
                { word: "賺", bopomofo: "ㄓㄨㄢˋ" },
                { word: "醃", bopomofo: "ㄧㄢ" },
                { word: "穎", bopomofo: "ㄧㄥˇ" },
                { word: "悔", bopomofo: "ㄏㄨㄟˇ" },
                { word: "默", bopomofo: "ㄇㄛˋ" },
                { word: "鬥", bopomofo: "ㄉㄡˋ" },
                { word: "扭", bopomofo: "ㄋㄧㄡˇ" },
                { word: "猛", bopomofo: "ㄇㄥˇ" },
                { word: "拱", bopomofo: "ㄍㄨㄥˇ" },
                { word: "距", bopomofo: "ㄐㄩˋ" },
                { word: "惠", bopomofo: "ㄏㄨㄟˋ" },
                { word: "凡", bopomofo: "ㄈㄢˊ" },
                { word: "悲", bopomofo: "ㄅㄟ" },
                { word: "愣", bopomofo: "ㄌㄥˋ" },
                { word: "勿", bopomofo: "ㄨˋ" },
                { word: "預", bopomofo: "ㄩˋ" },
                { word: "欠", bopomofo: "ㄑㄧㄢˋ" },
                { word: "狡", bopomofo: "ㄐㄧㄠˇ" },
                { word: "傲", bopomofo: "ㄠˋ" },
                { word: "昨", bopomofo: "ㄗㄨㄛˊ" },
                { word: "耘", bopomofo: "ㄩㄣˊ" },
                { word: "隔", bopomofo: "ㄍㄜˊ" },
                { word: "茁", bopomofo: "ㄓㄨㄛˊ" },
                { word: "者", bopomofo: "ㄓㄜˇ" },
                { word: "洒", bopomofo: "ㄙㄚˇ" },
                { word: "筍", bopomofo: "ㄙㄨㄣˇ" },
                { word: "掩", bopomofo: "ㄧㄢˇ" },
                { word: "覺", bopomofo: "ㄐㄩㄝˊ" },
                { word: "勺", bopomofo: "ㄕㄠˊ" },
                { word: "炸", bopomofo: "ㄓㄚˋ" },
                { word: "豬", bopomofo: "ㄓㄨ" },
                { word: "按", bopomofo: "ㄢˋ" },
                ...
                { word: "烹", bopomofo: "ㄆㄥ" },
                { word: "末", bopomofo: "ㄇㄛˋ" },
                { word: "勁", bopomofo: "ㄐㄧㄥˋ" },
                { word: "稠", bopomofo: "ㄔㄡˊ" },
                { word: "遂", bopomofo: "ㄙㄨㄟˋ" },
                { word: "敖", bopomofo: "ㄠˊ" },
                { word: "砂", bopomofo: "ㄕㄚ" },
                { word: "混", bopomofo: "ㄏㄨㄣˋ" },
                { word: "焦", bopomofo: "ㄐㄧㄠ" },
                { word: "舀", bopomofo: "ㄧㄠˇ" },
                { word: "脂", bopomofo: "ㄓ" },
                { word: "判", bopomofo: "ㄆㄢˋ" },
                { word: "騰", bopomofo: "ㄊㄥˊ" },
                { word: "熄", bopomofo: "ㄒㄧ" },
                { word: "啜", bopomofo: "ㄔㄨㄛˋ" },
                { word: "淌", bopomofo: "ㄊㄤˇ" },
                { word: "沛", bopomofo: "ㄆㄟˋ" },
                { word: "舔", bopomofo: "ㄊㄧㄢˇ" },
                { word: "呈", bopomofo: "ㄔㄥˊ" },
                { word: "慣", bopomofo: "ㄍㄨㄢˋ" },
                { word: "蹈", bopomofo: "ㄉㄠˇ" },
                { word: "徜", bopomofo: "ㄔㄤˊ" },
                { word: "徉", bopomofo: "ㄧㄤˊ" },
                { word: "又", bopomofo: "ㄧㄡˋ" },
                { word: "削", bopomofo: "ㄒㄩㄝ" },
                { word: "榜", bopomofo: "ㄅㄤˇ" },
                { word: "嫁", bopomofo: "ㄐㄧㄚˋ" },
                { word: "榮", bopomofo: "ㄖㄨㄥˊ" },
                { word: "濺", bopomofo: "ㄐㄧㄢˋ" },
                { word: "擺", bopomofo: "ㄅㄞˇ" },
                { word: "弄", bopomofo: "ㄋㄨㄥˋ" },
                { word: "恣", bopomofo: "ㄗˋ" },
                { word: "仰", bopomofo: "ㄧㄤˇ" },
                { word: "緬", bopomofo: "ㄇㄧㄢˇ" },
                { word: "甸", bopomofo: "ㄉㄧㄢˋ" },
                { word: "踝", bopomofo: "ㄏㄨㄞˊ" },
                { word: "絨", bopomofo: "ㄖㄨㄥˊ" },
                { word: "批", bopomofo: "ㄆㄧ" },
                { word: "徑", bopomofo: "ㄐㄧㄥˋ" },
                { word: "謹", bopomofo: "ㄐㄧㄣˇ" },
                { word: "恰", bopomofo: "ㄑㄧㄚˋ" },
                { word: "乏", bopomofo: "ㄈㄚˊ" },
                { word: "皆", bopomofo: "ㄐㄧㄝ" },
                { word: "嗤", bopomofo: "ㄔ" },
                { word: "隙", bopomofo: "ㄒㄧˋ" },
                { word: "竄", bopomofo: "ㄘㄨㄢˋ" },
                { word: "旨", bopomofo: "ㄓˇ" },
                { word: "悠", bopomofo: "ㄧㄡ" },
                { word: "寓", bopomofo: "ㄩˋ" },
                { word: "鷸", bopomofo: "ㄩˋ" },
                { word: "窟", bopomofo: "ㄎㄨ" },
                { word: "猾", bopomofo: "ㄏㄨㄚˊ" },
                { word: "騙", bopomofo: "ㄆㄧㄢˋ" },
                { word: "偽", bopomofo: "ㄨㄟˋ" },
                { word: "誇", bopomofo: "ㄎㄨㄚ" },
                { word: "辨", bopomofo: "ㄅㄧㄢˋ" },
                { word: "謊", bopomofo: "ㄏㄨㄤˇ" },
                { word: "諷", bopomofo: "ㄈㄥˋ" },
                { word: "惕", bopomofo: "ㄊㄧˋ" },
                { word: "察", bopomofo: "ㄔㄚˊ" },
                { word: "蔽", bopomofo: "ㄅㄧˋ" },
            ...
            level2: [
                { word: "游泳", bopomofo: "ㄧㄡˊ ㄩㄥˇ" },
                { word: "一般", bopomofo: "ㄧˋ ㄅㄢ" },
                
                { word: "滑行", bopomofo: "ㄏㄨㄚˊ ㄒㄧㄥˊ" },
                { word: "遨遊", bopomofo: "ㄠˊ ㄧㄡˊ" },
                { word: "緩緩", bopomofo: "ㄏㄨㄢˇ ㄏㄨㄢˇ" },
                { word: "陀螺", bopomofo: "ㄊㄨㄛˊ ㄌㄨㄛˊ" },
                { word: "轉身", bopomofo: "ㄓㄨㄢˇ ㄕㄣ" },
                { word: "跳躍", bopomofo: "ㄊㄧㄠˋ ㄩㄝˋ" },
                { word: "煩惱", bopomofo: "ㄈㄢˊ ㄋㄠˇ" },
                { word: "邀約", bopomofo: "ㄧㄠ ㄩㄝ" },
                { word: "反彈", bopomofo: "ㄈㄢˇ ㄊㄢˊ" },
                { word: "不曾", bopomofo: "ㄅㄨˋ ㄘㄥˊ" },
                { word: "指揮", bopomofo: "ㄓˇ ㄏㄨㄟ" },
                { word: "喧鬧", bopomofo: "ㄒㄩㄢ ㄋㄠˋ" },
                { word: "優美", bopomofo: "ㄧㄡ ㄇㄟˇ" },
                { word: "弧線", bopomofo: "ㄏㄨˊ ㄒㄧㄢˋ" },
                { word: "學業", bopomofo: "ㄒㄩㄝˊ ㄧㄝˋ" },
                { word: "勵志", bopomofo: "ㄌㄧˋ ㄓˋ" },
                { word: "寧靜", bopomofo: "ㄋㄧㄥˊ ㄐㄧㄥˋ" },
                { word: "視力", bopomofo: "ㄕˋ ㄌㄧˋ" },
                { word: "注意", bopomofo: "ㄓㄨˋ ㄧˋ" },
                { word: "單憑", bopomofo: "ㄉㄢ ㄆㄧㄥˊ" },
                { word: "蓓蕾", bopomofo: "ㄅㄟˋ ㄌㄟˇ" },
                { word: "任何", bopomofo: "ㄖㄣˋ ㄏㄜˊ" },
                { word: "地毯", bopomofo: "ㄉㄧˋ ㄊㄢˇ" },
                { word: "舒適", bopomofo: "ㄕㄨ ㄕˋ" },
                { word: "盡力", bopomofo: "ㄐㄧㄣˋ ㄌㄧˋ" },
                { word: "食糧", bopomofo: "ㄕˊ ㄌㄧㄤˊ" },
                { word: "爭吵", bopomofo: "ㄓㄥ ㄔㄠˇ" },
                { word: "警察", bopomofo: "ㄐㄧㄥˇ ㄔㄚˊ" },
                { word: "小偷", bopomofo: "ㄒㄧㄠˇ ㄊㄡ" },
                { word: "跳繩", bopomofo: "ㄊㄧㄠˋ ㄕㄥˊ" },
                { word: "結束", bopomofo: "ㄐㄧㄝˊ ㄕㄨˋ" },
                { word: "暫停", bopomofo: "ㄓㄢˋ ㄊㄧㄥˊ" },
                { word: "幸福", bopomofo: "ㄒㄧㄥˋ ㄈㄨˊ" },
                { word: "馬偕", bopomofo: "ㄇㄚˇ ㄐㄧㄝ" },
                { word: "醫院", bopomofo: "ㄧ ㄩㄢˋ" },
                { word: "生命", bopomofo: "ㄕㄥ ㄇㄧㄥˋ" },
                { word: "奉獻", bopomofo: "ㄈㄥˋ ㄒㄧㄢˋ" },
                { word: "醫療", bopomofo: "ㄧ ㄌㄧㄠˊ" },
                { word: "衛生", bopomofo: "ㄨㄟˋ ㄕㄥ" },
                { word: "藥品", bopomofo: "ㄧㄠˋ ㄆㄧㄣˇ" },
                { word: "治病", bopomofo: "ㄓˋ ㄅㄧㄥˋ" },
                { word: "捐錢", bopomofo: "ㄐㄩㄢ ㄑㄧㄢˊ" },
                { word: "食宿", bopomofo: "ㄕˊ ㄙㄨˋ" },
                { word: "學堂", bopomofo: "ㄒㄩㄝˊ ㄊㄤˊ" },
                { word: "濃濃", bopomofo: "ㄋㄨㄥˊ ㄋㄨㄥˊ" },
                { word: "開朗", bopomofo: "ㄎㄞ ㄌㄤˇ" },
                { word: "母親", bopomofo: "ㄇㄨˇ ㄑㄧㄣ" },
                { word: "冷酷", bopomofo: "ㄌㄥˇ ㄎㄨˋ" },
                { word: "磨練", bopomofo: "ㄇㄛˊ ㄌㄧㄢˋ" },
                { word: "攀登", bopomofo: "ㄆㄢ ㄉㄥ" },
                { word: "訓練", bopomofo: "ㄒㄩㄣˋ ㄌㄧㄢˋ" },
                { word: "谷底", bopomofo: "ㄍㄨˇ ㄉㄧˇ" },
                { word: "年級", bopomofo: "ㄋㄧㄢˊ ㄐㄧˊ" },
                { word: "壓軸", bopomofo: "ㄧㄚ ㄓㄡˊ" },
                { word: "爭奪", bopomofo: "ㄓㄥ ㄉㄨㄛˊ" },
                { word: "委屈", bopomofo: "ㄨㄟˇ ㄑㄩ" },
                { word: "偏心", bopomofo: "ㄆㄧㄢ ㄒㄧㄣ" },
                { word: "搬運", bopomofo: "ㄅㄢ ㄩㄣˋ" },
                { word: "怨恨", bopomofo: "ㄩㄢˋ ㄏㄣˋ" },
                { word: "抑制", bopomofo: "ㄧˋ ㄓˋ" },
                { word: "拒絕", bopomofo: "ㄐㄩˋ ㄐㄩㄝˊ" },
                { word: "尷尬", bopomofo: "ㄍㄢ ㄍㄚˋ" },
                { word: "後軌", bopomofo: "ㄏㄡˋ ㄍㄨㄟˇ" },
                { word: "距離", bopomofo: "ㄐㄩˋ ㄌㄧˊ" },
                { word: "默默", bopomofo: "ㄇㄛˋ ㄇㄛˋ" },
                { word: "鬥志", bopomofo: "ㄉㄡˋ ㄓˋ" },
                { word: "扭曲", bopomofo: "ㄋㄧㄡˇ ㄑㄩ" },
                { word: "猛然", bopomofo: "ㄇㄥˇ ㄖㄢˊ" },
                { word: "揭示", bopomofo: "ㄐㄧㄝ ㄕˋ" },
                { word: "卓越", bopomofo: "ㄓㄨㄛˊ ㄩㄝˋ" },
                { word: "培養", bopomofo: "ㄆㄟˊ ㄧㄤˇ" },
                { word: "總裁", bopomofo: "ㄗㄨㄥˇ ㄘㄞˊ" },
                { word: "瑣事", bopomofo: "ㄙㄨㄛˇ ㄕˋ" },
                { word: "耕田", bopomofo: "ㄍㄥ ㄊㄧㄢˊ" },
                { word: "賺錢", bopomofo: "ㄓㄨㄢˋ ㄑㄧㄢˊ" },
                { word: "聰穎", bopomofo: "ㄘㄨㄥ ㄧㄥˇ" },
                { word: "惋惜", bopomofo: "ㄨㄢˇ ㄒㄧ" },
                { word: "欠缺", bopomofo: "ㄑㄧㄢˋ ㄑㄩㄝ" },
                { word: "昨天", bopomofo: "ㄗㄨㄛˊ ㄊㄧㄢ" },
                { word: "耕耘", bopomofo: "ㄍㄥ ㄩㄣˊ" },
                { word: "茁壯", bopomofo: "ㄓㄨㄛˊ ㄓㄨㄤˋ" },
                { word: "泡沫", bopomofo: "ㄆㄠˋ ㄇㄛˋ" },
                { word: "熬煮", bopomofo: "ㄠˊ ㄓㄨˇ" },
                { word: "砂糖", bopomofo: "ㄕㄚ ㄊㄤˊ" },
                { word: "混合", bopomofo: "ㄏㄨㄣˋ ㄏㄜˊ" },
                { word: "焦糖", bopomofo: "ㄐㄧㄠ ㄊㄤˊ" },
                { word: "判斷", bopomofo: "ㄆㄢˋ ㄉㄨㄢˋ" },
                { word: "啜飲", bopomofo: "ㄔㄨㄛˋ ㄧㄣˇ" },
                { word: "流淌", bopomofo: "ㄌㄧㄡˊ ㄊㄤˇ" },
                { word: "豐沛", bopomofo: "ㄈㄥ ㄆㄟˋ" },
                { word: "綢繆", bopomofo: "ㄔㄡˊ ㄇㄡˊ" },
                { word: "掘井", bopomofo: "ㄐㄩㄝˊ ㄐㄧㄥˇ" },
                { word: "纏繞", bopomofo: "ㄔㄢˊ ㄖㄠˋ" },
                { word: "束縛", bopomofo: "ㄕㄨˋ ㄈㄨˊ" },
                { word: "瓦缶", bopomofo: "ㄨㄚˇ ㄈㄡˇ" },
                { word: "珍饈", bopomofo: "ㄓㄣ ㄒㄧㄡ" },
                { word: "施惠", bopomofo: "ㄕ ㄏㄨㄟˋ" },
                { word: "凡事", bopomofo: "ㄈㄢˊ ㄕˋ" },
                { word: "預留", bopomofo: "ㄩˋ ㄌㄧㄡˊ" },
                { word: "颳風", bopomofo: "ㄍㄨㄚ ㄈㄥ" },
                { word: "廚師", bopomofo: "ㄔㄨˊ ㄕ" },
                { word: "晒乾", bopomofo: "ㄕㄞˋ ㄍㄢ" },
                { word: "包覆", bopomofo: "ㄅㄠ ㄈㄨˋ" },
                { word: "竹筍", bopomofo: "ㄓㄨˊ ㄙㄨㄣˇ" },
                { word: "醃製", bopomofo: "ㄧㄢ ㄓˋ" },
                { word: "攪拌", bopomofo: "ㄐㄧㄠˇ ㄅㄢˋ" },
                { word: "油炸", bopomofo: "ㄧㄡˊ ㄓㄚˋ" },
                { word: "晶瑩", bopomofo: "ㄐㄧㄥ ㄧㄥˊ" },
                { word: "饗宴", bopomofo: "ㄒㄧㄤˇ ㄧㄢˋ" },
                { word: "彰化", bopomofo: "ㄓㄤ ㄏㄨㄚˋ" },
                { word: "豬肉", bopomofo: "ㄓㄨ ㄖㄡˋ" },
                { word: "歐洲", bopomofo: "ㄡ ㄓㄡ" },
                { word: "標榜", bopomofo: "ㄅㄧㄠ ㄅㄤˇ" },
                { word: "烹茶", bopomofo: "ㄆㄥ ㄔㄚˊ" },
                { word: "樹薯", bopomofo: "ㄕㄨˋ ㄕㄨˇ" },
                { word: "粉末", bopomofo: "ㄈㄣˇ ㄇㄛˋ" },
                { word: "黏稠", bopomofo: "ㄋㄧㄢˊ ㄔㄡˊ" },
                { word: "呈現", bopomofo: "ㄔㄥˊ ㄒㄧㄢˋ" },
                { word: "習慣", bopomofo: "ㄒㄧˊ ㄍㄨㄢˋ" },
                { word: "舞蹈", bopomofo: "ㄨˇ ㄉㄠˇ" },
                { word: "徜徉", bopomofo: "ㄔㄤˊ ㄧㄤˊ" }
            ],
            level3: [
                { word: "多采多姿", bopomofo: "ㄉㄨㄛ ㄘㄞˇ ㄉㄨㄛ ㄗ" },
                { word: "耳目一新", bopomofo: "ㄦˇ ㄇㄨˋ ㄧˋ ㄒㄧㄣ" },
                { word: "回心轉意", bopomofo: "ㄏㄨㄟˊ ㄒㄧㄣ ㄓㄨㄢˇ ㄧˋ" },
                { word: "躍躍欲試", bopomofo: "ㄩㄝˋ ㄩㄝˋ ㄩˋ ㄕˋ" },
                { word: "心煩意亂", bopomofo: "ㄒㄧㄣ ㄈㄢˊ ㄧˋ ㄌㄨㄢˋ" },
                { word: "遙不可及", bopomofo: "ㄧㄠˊ ㄅㄨˋ ㄎㄜˇ ㄐㄧˊ" },
                { word: "熟能生巧", bopomofo: "ㄕㄡˊ ㄋㄥˊ ㄕㄥ ㄑㄧㄠˇ" },
                { word: "此起彼落", bopomofo: "ㄘˇ ㄑㄧˇ ㄅㄧˇ ㄌㄨㄛˋ" },
                { word: "無與倫比", bopomofo: "ㄨˊ ㄩˇ ㄌㄨㄣˊ ㄅㄧˇ" },
                { word: "弄巧成拙", bopomofo: "ㄋㄨㄥˋ ㄑㄧㄠˇ ㄔㄥˊ ㄓㄨㄛ" },
                { word: "耳熟能詳", bopomofo: "ㄦˇ ㄕㄡˊ ㄋㄥˊ ㄒㄧㄤˊ" },
                { word: "對牛彈琴", bopomofo: "ㄉㄨㄟˋ ㄋㄧㄡˊ ㄊㄢˊ ㄑㄧㄣˊ" },
                { word: "五彩繽紛", bopomofo: "ㄨˇ ㄘㄞˇ ㄅㄧㄣ ㄈㄣ" },
                { word: "懸崖勒馬", bopomofo: "ㄒㄩㄢˊ ㄧㄚˊ ㄌㄜˋ ㄇㄚˇ" },
                { word: "孤注一擲", bopomofo: "ㄍㄨ ㄓㄨˋ ㄧˋ ㄓˊ" },
                { word: "雞犬不寧", bopomofo: "ㄐㄧ ㄑㄩㄢˇ ㄅㄨˋ ㄋㄧㄥˊ" },
                { word: "盡善盡美", bopomofo: "ㄐㄧㄣˋ ㄕㄢˋ ㄐㄧㄣˋ ㄇㄟˇ" },
                { word: "依依不捨", bopomofo: "ㄧ ㄧ ㄅㄨˋ ㄕㄜˇ" },
                { word: "赴湯蹈火", bopomofo: "ㄈㄨˋ ㄊㄤ ㄉㄠˇ ㄏㄨㄛˇ" },
                { word: "力爭上游", bopomofo: "ㄌㄧˋ ㄓㄥ ㄕㄤˋ ㄧㄡˊ" },
                { word: "以身作則", bopomofo: "ㄧˇ ㄕㄣ ㄗㄨㄛˋ ㄗㄜˊ" },
                { word: "束手無策", bopomofo: "ㄕㄨˋ ㄕㄡˇ ㄨˊ ㄘㄜˋ" },
                { word: "翻山越嶺", bopomofo: "ㄈㄢ ㄕㄢ ㄩㄝˋ ㄌㄧㄥˇ" },
                { word: "耳提面命", bopomofo: "ㄦˇ ㄊㄧˊ ㄇㄧㄢˋ ㄇㄧㄥˋ" },
                { word: "良藥苦口", bopomofo: "ㄌㄧㄤˊ ㄧㄠˋ ㄎㄨˇ ㄎㄡˇ" },
                { word: "精衛填海", bopomofo: "ㄐㄧㄥ ㄨㄟˋ ㄊㄧㄢˊ ㄏㄞˇ" },
                { word: "天翻地覆", bopomofo: "ㄊㄧㄢ ㄈㄢ ㄉㄧˋ ㄈㄨˋ" },
                { word: "登峰造極", bopomofo: "ㄉㄥ ㄈㄥ ㄗㄠˋ ㄐㄧˊ" },
                { word: "不同凡響", bopomofo: "ㄅㄨˋ ㄊㄨㄥˊ ㄈㄢˊ ㄒㄧㄤˇ" },
                { word: "豁然開朗", bopomofo: "ㄏㄨㄛˋ ㄖㄢˊ ㄎㄞ ㄌㄤˇ" },
                { word: "虛懷若谷", bopomofo: "ㄒㄩ ㄏㄨㄞˊ ㄖㄨㄛˋ ㄍㄨˇ" },
                { word: "拱手讓人", bopomofo: "ㄍㄨㄥˇ ㄕㄡˇ ㄖㄤˋ ㄖㄣˊ" },
                { word: "驕傲自滿", bopomofo: "ㄐㄧㄠ ㄠˋ ㄗˋ ㄇㄢˇ" },
                { word: "退避三舍", bopomofo: "ㄊㄨㄟˋ ㄅㄧˋ ㄙㄢ ㄕㄜˋ" },
                { word: "樂極生悲", bopomofo: "ㄌㄜˋ ㄐㄧˊ ㄕㄥ ㄅㄟ" }
            ],
            level4: [
                { word: "招招手", bopomofo: "ㄓㄠ ㄓㄠ ㄕㄡˇ" },
                { word: "珠穆朗瑪峰", bopomofo: "ㄓㄨ ㄇㄨˋ ㄌㄤˇ ㄇㄚˇ ㄈㄥ" },
                { word: "下午茶", bopomofo: "ㄒㄧㄚˋ ㄨˇ ㄔㄚˊ" },
                { word: "熱騰騰", bopomofo: "ㄖㄜˋ ㄊㄥˊ ㄊㄥˊ" },
                { word: "鼎邊趖", bopomofo: "ㄉㄧㄥˇ ㄅㄧㄢ ㄙㄨㄛ" },
                { word: "添加物", bopomofo: "ㄊㄧㄢ ㄐㄧㄚ ㄨˋ" }
            ]
        };
        ...
        var I = [{
            ...
            O = function() {
                function n(t, e, i) {
                    ! function(n, t) {
                        if (!(n instanceof t)) throw new TypeError("Cannot call a class as a function")
                    }(this, n), this.speed = M, this.type = l.a.checkArg(i, (function() {
                        // 指定機率：單字30%, 詞語50%, 成語15%, 短語5%
                        var rand = Math.random();
                        if (rand < 0.30) return 0; // 單字 (level1) 30%
                        else if (rand < 0.80) return 1; // 詞語 (level2) 50%
                        else if (rand < 0.95) return 2; // 成語 (level3) 15%
                        else return 3; // 短語 (level4) 5%
                    })()), this.w = I[this.type] ? I[this.type].w : 33, this.h = I[this.type] ? I[this.type].h : 50, this.x = l.a.checkArg(t, l.a.random(0, g.gameWidth)), this.y = l.a.checkArg(e, -this.h), this.words = A[this.type] && A[this.type].length > 0 ? A[this.type][l.a.random(0, A[this.type].length - 1)] : "測試", this.question = questionBank['level' + (this.type + 1)] ? questionBank['level' + (this.type + 1)].find(q => q.word === this.words) : null, this.bopomofo = this.question ? this.question.bopomofo.replace(/\s/g, '') : 'ㄘㄜˋㄕˋ', this.hp = this.bopomofo.length, this.hpBarW = 4 * this.hp, this.index = 0, this.img = r[this.type], this.deg = -Math.atan((this.x - c.player.x) / (this.y - c.player.y)), 0 == this.type ? this.speed += .2 : 3 == this.type && (this.sendMax = 15e3, this.sendIndex = 0), this.dieImg = f(), this.isDie = !1, this.dieIndex = -1, this.dieLen = 100, this.isRemove = !1
                }
                var t, e, i;
                return t = n, (e = [{
                    key: "act",
                    ...
                    key: "draw",
                    value: function() {
                        if (c.ctx.save(), c.ctx.translate(this.x, this.y), c.ctx.rotate(this.deg), c.ctx.translate(-this.x, -this.y), this.isDie) this.dieImg[this.dieIndex] && this.dieImg[this.dieIndex].complete && c.ctx.drawImage(this.dieImg[this.dieIndex], this.x - this.dieLen / 2, this.y - this.dieLen / 2, this.dieLen, this.dieLen);
                        else {
                            c.ctx.font = "20px MicrosoftYahei";
                            c.ctx.textAlign = "center"; // 確保文字置中對齊
                            c.ctx.textBaseline = "middle"; // 確保文字垂直置中
                            var n = this.y - this.h / 2;
                            this.img && this.img.complete && c.ctx.drawImage(this.img, this.x - this.w / 2, n, this.w, this.h);
                            
                            // 繪製注音符號（紅色，在敵機上方）
                            c.ctx.fillStyle = "#ff0000"; // 紅色
                            c.ctx.fillText(this.bopomofo.substring(0, this.bopomofo.length - this.hp), this.x, n - 15);
                            
                            // 繪製題目文字（白色，在敵機下方）
                            c.ctx.fillStyle = "#fff"; // 白色
                            n += this.h;
                            c.ctx.fillText(this.words, this.x, n + 20);
                            
                            // 繪製血條
                            c.ctx.fillStyle = "#f44";
                            c.ctx.fillRect(this.x - this.hpBarW / 2, n + 30, this.hpBarW, 3);
                            c.ctx.fillStyle = "#4f4";
                            c.ctx.fillRect(this.x - this.hpBarW / 2, n + 30, 4 * this.hp, 3);
                        }
                        c.ctx.restore()
                    }
                }, {
                    ...
                    key: "remove",
                    value: function() {
                    this.isRemove = !0, c.player.setScore(this), c.enemys.remove(this), c.enemys.sortByAttr("y", !1)
                    }
                }, {
                    key: "check",
                    value: function(n) {
        ...
        function V() {
            g.gameWidth = l.a.id("gameWrapper").wid(), g.gameHeight = 911, l.a.isMobile() ? (l.a.id("keyboard").css("display", "block"), g.gameHeight -= l.a.id("keyboard").hei()) : l.a.id("keyboard").css("display", "none"), c.canvas.width = g.gameWidth, c.canvas.height = g.gameHeight
        }
        _.a.use(X.a), window.J = l.a, l.a.ready((function() {
            l.a.isMobile() && (z = 15, function(n, t) {
                ...
                function() {
                    // i is now global
                    i.src = "./images/player.png";
                    i.onload = function() { };
                    i.onerror = function() { };
                    // o is now global
                    for (var n = 0; n < 2; n++) o[n] = new Image, o[n].src = "./images/player_err" + (n + 1) + ".png";
                    o[2] = o[1], o[3] = o[1], o[4] = o[1], o[5] = o[0];
                    // r is now global
                    for (var t = 0; t < 4; t++) r[t] = new Image, r[t].src = "./images/enemy" + (t + 1) + ".png";
                    // s is now global
                    for (var e = 0; e < 4; e++) s[e] = new Image, s[e].src = "./images/bomb" + (e + 1) + ".png";
                    // u is now global
                    for (var a = 0; a < 3; a++) u[a] = new Image, u[a].src = "./images/tail" + a + ".png";
                    // h is now global
                    h.src = "./images/bullet.png";
                }(), (function() {
                    // 確保遊戲尺寸在創建玩家之前設定
                    // 使用視窗尺寸而不是 gameWrapper，因為此時 gameWrapper 可能還是隱藏的
                    const gameWrapperWidth = window.innerWidth > 1200 ? 1000 : window.innerWidth - 20;
                    const gameWrapperHeight = 911; // 固定高度
                    g.gameWidth = gameWrapperWidth;
                    g.gameHeight = gameWrapperHeight;
                    console.log('修復遊戲尺寸:', gameWrapperWidth, 'x', gameWrapperHeight);
                })(), c.player = new L, c.enemys = new Array, c.bullets = new Array, c.isStop = false, c.isPause = false,                 window._clear = function() {
                    c.player.clear()
                    ...
                    J(n)
                }, l.a.cls("key-item").clk("_sendBullet(this.txt())"), c.bgMoveAni = new q({
                    ctx: c.ctx,
                ...
                document.getElementById('restartGameBtn') && document.getElementById('restartGameBtn').addEventListener('click', restartGame),
                document.getElementById('backToMenuBtn') && document.getElementById('backToMenuBtn').addEventListener('click', backToMenu),
                // 初始化畫面
                console.log('準備調用 initScreens...');
                initScreens();
                console.log('initScreens 調用完成');
        }));
        ...
        function R() {
            if (!c.isStop && !c.isPause) {
                // 檢查時間是否到
                if (c.player.isTimeUp()) {
                    c.isStop = true;
                    showEndScreen();
                    return;
                }
                ...
                // 清除 Canvas
                c.ctx.clearRect(0, 0, g.gameWidth, g.gameHeight);
                ...
                if (c.player.isClear) {
                    c.ctx.fillStyle = "#fff";
                    c.ctx.fillRect(0, c.player.y - c.player.h / 2 - c.player.clearIndex, g.gameWidth, 4);
                }
                
                // 繪製敵機
                c.enemys.each((function(n) {
                n.act()
                }));
                ...
                // 繪製計時器
                c.ctx.fillStyle = "#fff";
                c.ctx.font = "24px Arial";
                c.ctx.textAlign = "center";
                c.ctx.textBaseline = "top";
                var remainingTime = c.player.getRemainingTime();
                c.ctx.fillText("時間: " + remainingTime + "秒", g.gameWidth / 2, 20);
                ...
                c.ctx.fillStyle = "#fff";
                c.ctx.font = "48px Arial";
                c.ctx.textAlign = "center";
                c.ctx.textBaseline = "middle";
                c.ctx.fillText("遊戲暫停", g.gameWidth / 2, g.gameHeight / 2);
                
                c.ctx.font = "24px Arial";
                c.ctx.fillText("按暫停鍵繼續", g.gameWidth / 2, g.gameHeight / 2 + 60);
            }
            c.loopIndex++, M >= E || c.loopIndex % 200 == 0 && (M += .05)
        }

        // 難度遞增系統
        function updateDifficulty() {
            var elapsed = Math.floor((Date.now() - c.player.startTime) / 1000);
            var remaining = c.player.getRemainingTime();
            
            // 每10秒增加難度，但最後20秒有額外的激增
            var difficultyLevel = Math.floor((60 - remaining) / 10);
            var isLast20Seconds = remaining <= 20;
            
            // 敵機移動速度遞增 (基礎速度 0.4，每級 +0.2，最後20秒額外+0.3)
            var newEnemySpeed = 0.4 + (difficultyLevel * 0.2);
            if (isLast20Seconds) {
                newEnemySpeed += 0.3; // 最後20秒額外加速
            }
            if (newEnemySpeed !== M) {
                M = newEnemySpeed;
            }
            
            // 敵機生成間隔遞減 (基礎間隔 2000ms，每級 -300ms，最後20秒大幅縮短)
            var newAddEnemyTime = Math.max(800, 2000 - (difficultyLevel * 300));
            if (isLast20Seconds) {
                // 最後20秒：額外縮短到最低300ms
                newAddEnemyTime = Math.max(300, newAddEnemyTime - 300);
            }
            if (newAddEnemyTime !== c.addEnemyTime) {
                c.addEnemyTime = newAddEnemyTime;
                
                // 重新設定敵機生成間隔
                if (c.enemyInterval) {
                    clearInterval(c.enemyInterval);
                }
                c.enemyInterval = setInterval(function() {
                    if (!c.isStop && !c.isPause) {
                        c.enemys.append(new O());
                        console.log('新敵機已生成，當前敵機數量:', c.enemys.length);
                    }
                }, c.addEnemyTime);
            }
        }
        ...
        function B() {
            console.log('暫停函數 B() 被調用，c.isStop:', c.isStop, 'c.isPause:', c.isPause);
            if (c.isStop) {
                console.log('遊戲已停止，無法暫停');
                return;
            }
            if (c.isPause) {
                // 繼續遊戲
                ...
                // 計算暫停時間並累加到總暫停時間
                if (c.player.pauseStartTime > 0) {
                    var pauseDuration = Date.now() - c.player.pauseStartTime;
                    c.player.totalPauseTime += pauseDuration;
                    c.player.pauseStartTime = 0;
                    console.log('暫停時間:', pauseDuration, 'ms，總暫停時間:', c.player.totalPauseTime, 'ms');
                }
                
                l.a.id("pause").child(0).attr("src", "./images/pause.png");
                j(); // 隱藏暫停提示
                console.log('遊戲繼續');
            } else {
                // 暫停遊戲
                c.isPause = true;
                c.player.pauseStartTime = Date.now(); // 記錄暫停開始時間
                l.a.id("pause").child(0).attr("src", "./images/start.png");
                m("遊戲暫停中，按暫停鍵繼續", true, 3000); // 顯示暫停提示
                console.log('遊戲暫停');
            }
        }

        function J(n) {
            c.enemys.length > 0 ? c.bullets.append(new v(n)) : (m("沒有找到目標", !0, 1e3), c.player.setErr())
        }
        ...
        function geneEnemy() {
            ...
            // 立即生成第一個敵機
            c.enemys.append(new O());
            console.log('第一個敵機已生成');
            
            // 設置定時器持續生成敵機（使用新的間隔系統）
            c.enemyInterval = setInterval(() => {
                if (!c.isStop && !c.isPause) {
                    c.enemys.append(new O());
                    console.log('新敵機已生成，當前敵機數量:', c.enemys.length);
                }
        ...
        window.onkeydown = function(n) {
            ...
            // 移除按下 5 重新開始的功能 (keyCode 53)
            // 移除空白鍵暫停功能 (keyCode 32)
            ...
            if (c.isPause) {
                ...
                // 遊戲進行中的按鍵處理
                if (bopomofoKeyboard.isValidKey(key)) {
                    // 有效的注音符號按鍵 - 發射子彈
                    J(bopomofoKeyboard.keyToBopomofo(key));
                } else if (13 == t) { // Enter 鍵 - 護盾
                    c.player.clear();
                } else {
                    // 無效按鍵 - 顯示錯誤訊息
                    m("按鍵錯誤", !0, 1e3);
...
// 頁面可見性檢測 - 當用戶離開頁面時停止遊戲
document.addEventListener('visibilitychange', function() {
    if (document.hidden) {
        // 頁面隱藏時暫停遊戲
        if (window.c && !window.c.isStop && !window.c.isPause) {
            if (typeof window._pause === 'function') {
                window._pause();
            }
        }
    } else {
...
window.addEventListener('beforeunload', function() {
    if (window.c) {
        window.c.isStop = true;
        // 清除所有定時器
        if (window.c.enemyInterval) {
            clearInterval(window.c.enemyInterval);
        }
    }
});
...
function showEndScreen() {
    ...
    if (endScreen) {
        ...
        if (window.c && window.c.player) {
            playerScore = window.c.player.score || 0;
            playerBestScore = window.c.player.bestScore || 0;
        } else {
            // 備用方案：從 DOM 元素讀取
            ...
            if (scoreElement) {
                playerScore = parseInt(scoreElement.textContent) || 0;
            }
            if (bestScoreElement) {
                playerBestScore = parseInt(bestScoreElement.textContent) || 0;
            }
        }
        ...
        const finalBestScore = document.getElementById('finalBestScore');
        if (finalBestScore) {
            finalBestScore.textContent = playerBestScore;
        }
        ...
        // 調試：檢查分數獲取情況
        console.log('分數獲取調試:', {
            window_c_exists: !!window.c,
            window_c_player_exists: !!(window.c && window.c.player),
            playerScore_from_window: window.c && window.c.player ? window.c.player.score : 'N/A',
            scoreElement_exists: !!document.getElementById('score'),
            scoreElement_value: document.getElementById('score') ? document.getElementById('score').textContent : 'N/A'
        });
...
document.addEventListener('keydown', function(event) {
    if (event.key === 'Escape') {
        ...
        if (gameWrapper && gameWrapper.style.display !== 'none') {
            // 停止遊戲
            if (window.c && window.c.isStop !== undefined) {
                window.c.isStop = true;
            }
...
// 獲取並顯示個人最高分
async function fetchAndDisplayPersonalBest(currentScore) {
    try {
        // 獲取個人最高分
        const response = await fetch('/api/games/personal-best?game_type=typing_hero');
        ...
        // 如果獲取失敗，只顯示當前分數
        updatePersonalBestDisplay(currentScore, 0);
    }
}

// 更新個人最高分顯示
function updatePersonalBestDisplay(currentScore, personalBest) {
    const finalBestScore = document.getElementById('finalBestScore');
    if (finalBestScore) {
        if (currentScore > personalBest) {
            // 新紀錄！
            finalBestScore.innerHTML = `
